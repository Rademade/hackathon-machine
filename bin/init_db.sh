...
if  [ $(docker-compose -f $DOCKER_INIT_DB_CONFIG ps | grep dbdata | wc -l) == 0 ]; then
    ...
    docker-compose -f $DOCKER_INIT_DB_CONFIG up -d db
    DB_CONTAINER_ID=$(docker-compose -f $DOCKER_INIT_DB_CONFIG ps -q db)
    for i in {30..0}; do
        ...
        if [ $(docker logs $DB_CONTAINER_ID 2>&1 | grep "database system is ready to accept connections" | wc -l) == 1 ]; then
            docker-compose -f $DOCKER_INIT_DB_CONFIG stop db
            echo "db initialized"
            exit 0
        else
            sleep 1
